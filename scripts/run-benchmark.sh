...
bind_repository_root "scripts" "run-benchmark.sh"
if [[ ! -f pyproject.toml || ! -f model-lock.json ]]; then
  echo "Could not verify the OneLoad repository root." >&2
  exit 65
fi
model_dir="${ONELOAD_MODEL_DIR:-}"

if [[ -z "${model_dir}" ]]; then
  echo "Set ONELOAD_MODEL_DIR to the pinned local model directory." >&2
  exit 64
fi

python_bin="./.venv/bin/python"
if [[ ! -x "${python_bin}" ]]; then
  echo "Run uv sync --frozen --python 3.12 before benchmarking." >&2
  exit 69
fi
